    ...
    <SiteLayoutClient activePage="qloud">
      <div className="space-y-12 pb-20">
        ...
        <section className="space-y-8">
           ...
           {fetching ? (
             <div className="py-20 flex justify-center italic font-black text-foreground/40 animate-pulse uppercase tracking-[0.5em]">Lade Nodes...</div>
           ) : myGroups.length > 0 ? (
                      ...
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="group p-8 bg-card border border-border rounded-[2.5rem] hover:border-primary/40 transition-all cursor-pointer shadow-sm hover:shadow-2xl relative overflow-hidden">
                         <div className="relative z-10 flex flex-col gap-6">
                            <div className="flex items-center justify-between">
                               ...
                               <span className="text-[10px] font-black uppercase text-foreground/40 italic tracking-widest">{group._count.members} User</span>
                            </div>
                            <div>
             ...
             <div className="py-20 flex flex-col items-center text-center space-y-6">
                <div className="w-24 h-24 bg-foreground/5 rounded-full flex items-center justify-center border border-dashed border-foreground/20 italic font-black text-foreground/20 uppercase tracking-widest text-[10px]">Empty</div>
                <p className="font-black italic uppercase text-foreground/30 tracking-widest text-sm">Bereit für deine erste Qloud?</p>
             </div>
           )}
        </section>
      </div>
      ...
      <AnimatePresence>
        {isJoinOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
             ...
             <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="relative w-full max-w-2xl bg-card text-foreground shadow-2xl rounded-[3rem] overflow-hidden flex flex-col border border-border">
                <div className="flex-1 p-10 space-y-8 overflow-y-auto max-h-[85vh]">
                   <header className="flex items-center justify-between">
                      <div>
                         <h2 className="text-4xl font-black italic uppercase tracking-tighter leading-none">Find your <br /><span className="text-primary tracking-widest">Qloud.</span></h2>
                         <div className="mt-4 flex gap-4">
                            <button onClick={() => setIsScanning(false)} className={`text-[10px] font-black uppercase italic tracking-widest pb-1 border-b-2 transition-all ${!isScanning ? 'border-primary text-foreground' : 'border-transparent text-foreground/20'}`}>Suche</button>
                            <button onClick={() => setIsScanning(true)} className={`text-[10px] font-black uppercase italic tracking-widest pb-1 border-b-2 transition-all ${isScanning ? 'border-primary text-foreground' : 'border-transparent text-foreground/20'}`}>Scan</button>
                         </div>
                      </div>
      ...
      <AnimatePresence>
        {isCreateOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
             ...
             <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} className="relative w-full max-w-lg bg-card border border-border rounded-[3.5rem] p-10 shadow-2xl overflow-hidden" >
                <div className="space-y-8">
                   <div className="flex items-center gap-4 text-foreground">
                      <div className="p-4 bg-primary rounded-2xl text-secondary shadow-lg shadow-primary/20"><Plus size={24} /></div>
                      <div>
                   ...
                   <form onSubmit={handleCreateGroup} className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <input 
                          type="text" 
                          required 
                          placeholder="GÜLER WEDDING, ETC..." 
                          value={groupName} 
                          onChange={(e) => setGroupName(e.target.value)} 
                          className="w-full bg-foreground/5 border border-border rounded-2xl py-6 px-8 font-black italic uppercase text-sm outline-none focus:border-primary transition-all text-foreground placeholder:text-foreground/20" 
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase italic text-foreground/30 ml-2 tracking-widest">Mission Context</label>
                        <textarea 
                          required 
                          placeholder="DETAILS ZUM EVENT..." 
                          value={groupDesc} 
                          onChange={(e) => setGroupDesc(e.target.value)} 
                          className="w-full bg-foreground/5 border border-border rounded-2xl py-6 px-8 font-medium italic text-sm outline-none focus:border-primary transition-all min-h-[120px] text-foreground placeholder:text-foreground/20" 
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-4 pt-4">
                         <button type="button" onClick={() => setIsCreateOpen(false)} className="py-6 bg-foreground/5 text-foreground/40 font-black italic uppercase tracking-widest text-[10px] rounded-2xl hover:bg-foreground/10 transition-all border border-border">Abort</button>
                         <button type="submit" disabled={isLoading} className={`py-6 font-black italic uppercase tracking-widest text-[10px] rounded-2xl transition-all shadow-xl ${isFormValid ? 'bg-primary text-secondary shadow-primary/20' : 'bg-foreground/5 text-foreground/10 cursor-not-allowed grayscale'}`}>{isLoading ? "Syncing..." : "Launch Qloud"}</button>
                      </div>
                   </form>
                </div>
             </motion.div>
